/* sw.js simple cache */
// Use a versioned cache name for the service worker.  Bump this when
// updating cached files.  Paths below are relative so the app works from
// subdirectories (e.g. /Test1/).
const CACHE = 'csc-v1-r50';
const CORE = [
  './',
  'index.html', 'styles.css', 'app.js', 'sfx.js',
  'manifest.webmanifest', 'logo.png', 'favicon.ico',
  'demo-week-schedule.json', 'sounds/manifest.json',
  'sounds/click.mp3', 'sounds/a.mp3'
];
self.addEventListener('install', e => {
  self.skipWaiting();
  e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE).catch(()=>{})));
});
self.addEventListener('activate', e => {
  e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))));
  self.clients.claim();
});
self.addEventListener('fetch', e => {
  const req = e.request;
  if (req.method !== 'GET') return;
  e.respondWith(
    caches.match(req).then(cached => {
      const fetchPromise = fetch(req).then(resp => {
        const copy = resp.clone();
        caches.open(CACHE).then(c=>{ if(resp.ok) c.put(req, copy); }).catch(()=>{});
        return resp;
      }).catch(()=>cached || new Response('Offline', {status:503}));
      return cached || fetchPromise;
    })
  );
});
// Replace innerHTML with DOM creation to prevent XSS
// Original (line ~1234): div.innerHTML = `<span>${displayTime(ev.time)} – ${ev.type}</span>...`;
// Suggested replacement:
const span = document.createElement('span');
span.textContent = `${displayTime(ev.time)} – ${ev.type}`;
div.appendChild(span);
// Add similar for other innerHTML usages (e.g., in renderSchedule, renderWeek).
